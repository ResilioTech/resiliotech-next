...
- Which model belongs on which GPU class? (See our guide on [Capacity Planning for LLM Inference](/blog/capacity-planning-llm-inference-gpu-memory-throughput-slas))
...
If you're using KEDA or a custom metrics adapter, scale on demand signals that match user pain. To understand the relationship between concurrency and GPU efficiency, see our detailed breakdown of [Batching Strategies for LLM Inference](/blog/batching-strategies-llm-inference-throughput-vs-latency).
...
our current serving architecture.*
